...
export default function Edit(props) {
    ...
    if (isResolving) {
        return (
            <div {...blockProps}>
                <Placeholder label={__('Taxonomy Menu Accordion', 'texonomy-menu-accordion')}>
                    <Spinner />
                    <p>{__('Loading categories...', 'texonomy-menu-accordion')}</p>
                </Placeholder>
            </div>
        );
    }
    ...
    return (
        <div {...blockProps}>
            ...
            <div className="ea-main-editor-container" style={{ border: '1px solid #2c2c2c', borderRadius: '4px' }}>
                {treeData.length > 0 ? (
                    // ✅ Pass template attribute to renderList
                    renderList(treeData, openItems, toggleItem, attributes.template || 0)
                ) : (
                    <p style={{ padding: '20px' }}>{__('No categories found.', 'texonomy-menu-accordion')}</p>
                )}
            </div>
        </div>
    );
